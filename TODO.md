...
| 批次 3（**进行中**：A4、B2、C3、C5、C6 已完成，剩 B7/B8） | 中功能 | A4 ✅、B2 ✅、B7、B8、C3 ✅、C5 ✅、C6 ✅ | 0.11.0（MINOR） |
| **F1 专项** | 交互规范统一 | **F1** | **0.12.0（MINOR，独立版本）** |
| 批次 4 | Phase 2 大件 | A2、B1、B4、B5、B6、C4、D5、E1、E2 | 0.13.0+ |
| 批次 5 | Phase 3/4 收尾 | D6–D9、E3–E5 | 0.14.0+ |
...
- [x] **A4 `/whois` 查询结果一律走私信（含「未找到映射」）**（P1 · 批次3 · 已完成，待发版）
...
- [x] **B2 命中广告规则时发完整卡片并 @ 当事人**（P1 · 批次3 · 已完成，待发版）命中后被动回复一张卡片：
  @ 当事人（卡片内 `<@!openid>`）+ 命中规则 + 实际处理动作（含禁言时长）+ 群规则文案；
  未注入富消息发送器时退化为纯文本警告。**申诉入口按钮待 B8 落地时一起接。**
...
- [ ] **B7 审核事件订阅（机器人处罚事件推送）**（P1 · 批次3 · Phase 2 前置）审核员及以上可订阅本群的**审核事件**：
  机器人执行处罚（撤回 / 警告 / 禁言 / 移出 / 拉黑）时，给订阅者私信一张卡——**动作 + 完整原消息 + 当事人与操作者**；
  订阅维度按「用户 + 群 + 事件类型」存（新增 `event_subscriptions` 表，或复用 `notification_subscriptions`
  的 scope 编码 `event:<type>:<群ID>`），投递复用 `PushService` 去重。
...
- [ ] 真机确认：官方是否存在「用户撤回消息」的下行事件（决定 B7 能否扩到这一口径）（真机 · 批次2）
- [ ] **B8 申诉能力**（P2 · 批次3 · Phase 3，澄清已就位）
  - 触发：机器人执行处罚（撤回 / 禁言 / 移出 / 拉黑）时，**私聊当事人**一张申诉卡，含**只可触发一次**的
    申诉按钮，并写明「恶意申诉可能加重处罚」；**仅违规本人**可发起（他人发起一律拒绝并说明）；
  - 审批：申诉事件私信给 B7 的订阅者（B7 未落地时先发给该群审核员及以上），管理员在卡上「通过 / 驳回」；
  - 通过后**按当时实际执行的处罚逐项撤销**：撤回消息 / 解禁 / 解除拉黑（取决于官方能力与 audit 里记录的动作）；
  - 依赖：新增「处罚记录 + 申诉状态」表（含 `audit` 行 id / 动作 / messageId / 一次性标记），
    以及解禁与解除拉黑的官方接口核对（`muteGroupMember` 秒数 0 / `updateMemberBlacklist` 移除）。
...
- [x] **C3 活动定时提醒**（P1 · 批次3 · 已完成，待发版）`/activity set <短码> remindAt MM-DD HH:mm`
...
- [ ] **F1 群内回复统一 @ 具体发起人**（P1 · F1 专项（独立版本 0.12.0） · 横切）**所有群内回复**——操作确认、菜单、反馈、
