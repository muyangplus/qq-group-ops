...
| 批次 2（待你安排真机） | 验收与交付验证 | B3、D1–D4、C 节两条真机确认 | 不发版 |
| 批次 3（**已发布 0.11.0**） | 中功能 | A4 ✅、B2 ✅、C3 ✅、C5 ✅、C6 ✅ | 0.11.0 ✅ |
| 批次 3b | 审核事件订阅 + 申诉 + 黑名单 | A5、B7、B8 | 0.12.0（MINOR） |
| **F1 专项** | 交互规范统一 | **F1** | **0.13.0（MINOR，独立版本）** |
| 批次 4 | Phase 2 大件 | A2、B1、B4、B5、B6、C4、D5、E1、E2 | 0.14.0+ |
| 批次 5 | Phase 3/4 收尾 | D6–D9、E3–E5 | 0.15.0+ |
...
- [ ] **A5 黑名单（本群 / 全局）+ 入群审批最高优先级拦截**（P1 · 批次3b · 与 B7/B8 的卡片动作配套）
  - 作用域：**本群**（只影响该群）与**全局**（影响机器人已绑定的所有群）；
  - **入群审批最高优先级**：命中黑名单的申请人**直接不通过**，先于 `joinDecision` / 名单筛选 / 班级姓名等一切规则；
  - **已在对应区域内则踢出**：命中本群黑名单 → 在该群踢出；命中全局黑名单 → 在其所在的**所有绑定群**踢出；
  - 需要：黑名单表（作用域 + userId + 来源/操作者/时间）、管理入口（如 `/blacklist add|del|list [本群|全局]`）、
    审计留痕，以及和官方 `add_to_member_blacklist`（群级拉黑接口）的分工。
...
- [ ] **B7 审核事件订阅（机器人处罚事件推送）**（P1 · 批次3 · Phase 2 前置）审核员及以上可订阅本群的**审核事件**：
  ...
  - **卡片动作**：订阅的审核员及以上可**直接在私信卡片上调整处罚**（与申诉一致）：
...
- [ ] **B8 申诉能力**（P2 · 批次3 · Phase 3，澄清已就位）
  ...
  - 审批：申诉事件私信给 B7 的订阅者（B7 未落地时先发给该群审核员及以上），管理员在卡上**直接调整处罚**：
    **解除处罚 / 修改禁言时长 / 踢出 / 拉黑（本群或全局，见 A5）**；
  - 解除处罚时**按当时实际执行的处罚逐项撤销**：撤回消息 / 解禁 / 解除拉黑（取决于官方能力与 audit 记录的动作）；
