...
describe("activity card callbacks (§B2)", async () => {
  ...
  function withActivity(): {
    ...
    activityNotifications = new ActivityNotificationService(notifier, undefined, undefined, {
      dailyLimit: 0,
    });
  ...
  it("replies to a group join callback with a mention-only receipt", async () => {
  ...
  it("sends the failure reason as a DM and keeps it out of the group", async () => {
    const { svc, profiles } = withActivity();
    ...
    expect(result.text).toContain("原因已私信");
    expect(result.text).not.toContain("仅限");
  ...
  it("reports a waitlist position when the activity is full", async () => {
    ...
    expect(waitlisted.text).toContain("已进入候补 · 第 1 位");
    expect(waitlisted.text).toContain("<@!u4>");
    expect(waitlisted.text).not.toContain("同学乙");
  });
  ...
  it("freezes the slot on quit, then promotes on release", async () => {
    ...
    expect(quit.text).toContain("待释放名额 1");
    expect(quit.text).toContain("<@!u3>");
  ...
  it("automatically promotes the next waitlist entry in auto mode", async () => {
    ...
    const quit = await svc.activityCallbackCard("quit", ["#ACT001"], "u3", "g1");
    expect(quit.ok).toBe(true);
    expect(quit.text).toContain("已自动递补候补第一位");
    const dm = String(api.sentPrivateMessages.at(-1)?.markdown ?? "");
    expect(dm).toContain("你已递补成功");
    // auto 模式不会冻结名额
    expect(quit.text).not.toContain("待释放");
  });
