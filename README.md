...
请审核：点击下方按钮，或发送 `/approve <group> <id>` / `/reject <group> <id> <原因>`
[ 同意 ] [ 拒绝 ]
```

「同意 / 拒绝」是**指令按钮**：点击后自动发送 `/approve <group_openid> <申请ID>` / `/reject <group_openid> <申请ID> 审核未通过`，并带二次确认弹窗。按钮走的是与手动输入**完全相同**的指令与权限校验，不存在绕过。
