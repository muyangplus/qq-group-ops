...
- 已实现：配置、领域模型、规则引擎、审计日志、权限模型、多群配置、入群审核状态机、入群申请同步、消息审核执行、事件路由、事件网关抽象、WebSocket 网关骨架、事件映射器、运行时装配、Phase 0 检查核心、PostgreSQL schema 与审计/入群申请/群配置仓储、管理员命令、活动报名、信息导出、官方 API 客户端与测试替身。
- 待实现：真实官方事件格式映射、WebSocket 生产连接、PostgreSQL 生产连接与迁移、Web 管理后台、内容安全与 AI 辅助。
- 测试：Vitest，共 95 个测试。
