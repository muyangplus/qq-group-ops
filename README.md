...
「群里 @ 到人」在官方能力里有两种发法，效果**必须真机确认**，所以提供一个自检指令（仅全局超管，在群里执行）：

```text
/testat           发 3 条测试消息：纯文本 @、Markdown 首行 @、Markdown 正文中间 @
/testat all       额外再发一条纯文本 @everyone（会真的 @ 全群，谨慎）
```

发完会回一张汇总卡，请你回复「**哪几条真的 @ 到了你**」（昵称高亮 / 收到提醒 / 收到通知），据此统一项目里所有 @ 的实现方式。

官方能力边界（已按内嵌格式文档核对）：

| 项 | 官方现状 | 本项目的处理 |
|---|---|---|
| 内嵌格式生效范围 | `<@!openid>`（@某人）与 `@everyone`（@所有人）**只在 `content` 里生效**；Ark / Embed 不生效 | `/testat` 第 1、4 条走**纯文本 `content`** 通道 |
| Markdown 卡片 | `msg_type=2` 的 payload 只有 `markdown.content`，**没有 `content` 字段** | 卡片里的 @ 是否有效由 `/testat` 第 2、3 条实测 |
| `@everyone` 权限 | 文档注明「需要机器人拥有发送 @所有人 消息的权限」 | 失败时汇总卡直接给出错误原因，不静默 |
| @ 的降级路径 | —— | 若实测「纯文本才生效」，则所有需要真 @ 的场景统一改成「纯文本 @ +（可选）卡片」两条消息 |
