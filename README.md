...
- 回调链路：新增 `INTERACTION (1<<26)` intent 与 `INTERACTION_CREATE` 事件处理，`/testmenu` 用回调按钮翻页（回包 `PUT /interactions/{id}` + 被动回复新一页，失败自动改主动发送）。
...
| 必须回包 | 收到后要调 `PUT /interactions/{id}` 回 `{"code":0}`，否则客户端一直 loading 到超时；同一 id 只能回一次 | 收到立即回包；回包失败只记日志，卡片照发 |
| **更新原消息** | ❌ **没有该接口**（消息类只有发送与撤回） | 翻页 = 拿 interaction id 当 `msg_id` **被动回复一条新消息**，不是改写原卡片 |
| 发送失败 | —— | 自动降级为主动发送（`RichMessageSender` 三级降级），用户仍能看到新的一页 |
| 双通道兜底 | —— | 正文与第二行按钮保留 `/testmenu <页码>`，互动事件没开通也能手动翻页 |

> 结论：QQ 官方**做不出**"原地改写同一张卡"的翻页；能做出的是"点一下就回复下一页"，
> 体验上仍然比指令按钮好（用户不用手输）。别把这里的实现描述成原地更新。
