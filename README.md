...
- 事件通道（0.17.0）：**WebSocket（默认）与 Webhook 二选一**（`EVENT_MODE`）——Webhook 走 Fastify 回调 + 官方 Ed25519 验签（`X-Signature-Ed25519` / `X-Signature-Timestamp`，`op=13` 握手回签），先回 ACK 再按序处理；部署与排查见 [docs/OPERATIONS.md](docs/OPERATIONS.md)。
- 待实现：真实环境联调、Web 管理后台、内容安全与 AI 辅助。
- 审核与申诉（0.12.0）：**黑名单（本群 / 全局）**——入群审批最高优先级直接拒绝、已在群内踢出，本群黑名单同时调官方群拉黑接口；**处罚事件推送**——关键词实际处罚后私信推送给订阅者，卡片上可直接**解除处罚 / 改禁言时长 / 踢出 / 拉黑**；**申诉**——`/appeal` 或群内警告卡的「我要申诉」按钮，审核员在卡片上通过 / 驳回或调整处罚；处罚通知用 `/notify punish` 独立订阅（详见 [docs/COMMANDS.md](docs/COMMANDS.md)）。
- 交互菜单（§F2）：`/menu` 默认出**普通成员常用菜单**（帮助 / 我的资料 / 我的权限 / 活动 / 申诉）；管理 / 超管入口只在私信显示按钮，群里需手动 `/menu 管理`，**超管菜单仅限私信**；按钮即指令或回调、自动三级降级；空 `@机器人`、未知指令、私信首次交互都会回到常用菜单。
...
- 卡片标准：**所有指令输出统一为菜单式卡片**（导航/查看/开关/枚举用回调自动完成、需要参数或不可逆的动作用指令按钮、列表用回调翻页 + `+页码` 降级）；**群内回复统一在卡片首行 @ 发起人**（§F1，发送层统一处理，`test` 模块豁免、私聊不加、纯文本降级通道不支持提及），规范见 [docs/CARD-STANDARD.md](docs/CARD-STANDARD.md)；已迁移 `/help`、`/status`、`/pending`、`/rules`、`/audit`、`/test`、`/sync`、`/approve`、`/reject`、`/notify`、`/punish`、`/appeal`、`/blacklist`，其余指令按批次迁移。
- 消息审核（§B1）：关键词之外支持**正则规则**（与关键词同一条处罚管道）与**用户白名单**；`/rules set regexRules|userWhitelist`、`/rules add|del regex|whitelist`；审核日志用 `/export audit [数量]` 导出（脱敏、只私信）。
...
- 测试：Vitest，共 746 个测试（含端到端验收干跑；SQLite 与 PostgreSQL 方言均覆盖）。
...
- `/bind` 绑定 QQ号 / 群号
- 强制绑定 QQ 号和群号后才能使用（`/help`、`/bind`、`/appeal` 除外）
...
- 活动报名：活动发布、报名收集、名单管理、统计（签到统计**已决定不做**）
...
| `/punish`（处罚记录与卡片改处罚） | ✅ | ✅（仅本群） | ✅（仅本群） | ✅（仅本群） |
| `/blacklist`（本群黑名单） | ✅ | ✅（仅本群） | ✅（仅本群） | ✅（仅本群） |
| `/blacklist 全局`（全局黑名单） | ✅ | ❌ | ❌ | ❌ |
| `/appeal`（申诉自己的处罚） | ✅ | ✅ | ✅ | ✅ |
...
- [CD：打 tag 发 Release 自动发布到 FTP（含 GitHub 配置清单与安全审计）](docs/CD.md)
