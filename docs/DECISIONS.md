...
- 状态：已决定不做（2026-09-26，用户确认）
- 背景：`ROADMAP.md` Phase 2 与 `README.md` 的规划里一直挂着「活动报名 → 签到统计」；
  报名模块已具备「报名 / 候补 / 满员广播 / 定时提醒 / 名单 / 统计图片 / CSV 导出」全链路。
- 决策：**不做签到**。不新增 `activity_checkins` 表、不加签到按钮 / 口令核销 / 管理员勾选，
  `TODO.md` 与 `ROADMAP.md` 里的该条目移除（保留本 ADR 记录，避免以后又被重新加入排期）。
- 理由：
  1. 报名名单 + 统计 + CSV 导出已经覆盖「谁参加、来了多少人」的报账需求，签到是**第二套数据源**；
  2. 签到天然要求「活动现场实时操作」，与机器人被动响应模型不匹配（要额外做时间窗、核销、补签）；
  3. QQ 官方没有扫码 / 定位类能力，最省事的「私信一键签到」可被代签，防伪强度与实现成本不成正比。
- 影响：`TODO.md`（C4 移除，仅留取消说明）、`ROADMAP.md`、`README.md`；
  真机清单不需要新增验证项。已实现的活动能力（含 `mentionAll` 能力边界）不受影响。

## ADR-0045：`@全体` 能力边界与群内 @全体 的短路处理（§B3）

- 状态：已采纳（0.15.1）
- 背景：真机抓包（R1）确认群里别人发「@全体成员」在事件里的原文是 **`<@all> `**（`@everyone` 则是
  `@everyone`）。它会被原有的 `stripBotMention` 当成「@机器人」剥掉，只剩空内容，
  于是被当成「空 @机器人」**回一张常用菜单** —— 群里任何人 @全体 都会触发机器人抢答。
- 决策：
  1. **入站**：开头的 `@全体` 标记（`<@all>` / `<@!all>` / `@everyone` / `@全体成员` / `@所有人` 等）
     一律无条件剥离；**整条只有 @全体、没有正文**时判为群广播，`mapGroupMessage` 直接返回 `null`
     （不回复、不送关键词审核、不写审计、不处罚）。带正文的照常审核，`<@all> /menu` 照常执行；
  2. **出站**：机器人**无法 @全体** —— 现场穷举 8 种写法（含官方入站原文形态 `<@all>` 的卡片与纯文本、
     纯文本 `@everyone`）**全部提醒不到任何人**；因此活动 `mentionAll on` 只提示操作者手动 @ 一条，
     不假装能 @（配置卡与 `/help activity` 均如实写明）。
- 理由：入站短路是修复「抢答」的最小改动，且 `@全体` 本身没有正文可审核；
  出站能力边界经实测确认，写进文档可避免重复试错。
...
- 决策（用户逐条确认）：
  ...
  4. **申诉卡片补按钮**：我要申诉引导卡 = 「**直接提交**」（**回调**，一键建无理由单）+
     「写理由提交」（指令按钮，预填 `/appeal #码 `）；申诉已提交回执 = 「补充理由」；
     私信卡片的按钮开关改看 **user** 目标的键盘可用性（此前误用群目标，群键盘被拒会连累私信卡片）。
     **主按钮必须是回调**：被禁言的当事人点指令按钮会被 QQ 客户端拦住（PC 端弹「禁言中，请解禁后再试」），
     回调不经过客户端发送，因此禁言状态下依旧可用；
     **当事人卡片不放审核员按钮**：`cb:punish:view` 要求审核员及以上，当事人点了只会得到「无权限操作」，
     所以我要申诉 / 申诉已提交两张卡都不再出现「查看处罚」；
  5. **申诉降级**：点「我要申诉」而私信发不出去时，**不再丢单** —— 直接按「无理由」建单并照常通知审核员，
     群里只回一条**不含申诉内容**的提示（与 `/whois` 私信失败的口径一致）；
  6. **原文渲染格式**：放在**标题下第一段**，标签行 + Markdown 引用段落（`原文：` / `> 违规内容全文`），
     未保留时只给一行「（未保留原文）」；引用块**后面必须补空行**，否则 Markdown 会把后续字段
     当成引用块的懒惰延续（真机踩过）。
  7. **禁言约束（真机结论）**：成员被禁言时，**群里的任何按钮都点不动、指令按钮也发不出去**
     （QQ 客户端直接拦，PC 端提示「禁言中，请解禁后再试」）。因此：
     - 群内「处罚通知」卡的 footer 必须给出**私聊申诉**的等价指令
       （「点击下方按钮或私聊机器人发送 `/appeal #短码 <理由>` 即可申诉」）——
       这是 v2「卡片不写指令」的明确例外，因为对应按钮在该状态下不可用；能点按钮的人照旧一键进引导卡；
     - 当事人私信卡片的按钮**不带** `permission.specifyUserIds`（跨群 / 单聊 openid 归属不一致时
       客户端会判「无权限操作」），鉴权全部留在服务端；群内广播卡保留该限制，避免别人替当事人点。
