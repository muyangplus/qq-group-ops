...
| # | 待确认 | 怎么测 | 预期/观察点 |
|---|---|---|---|
| R1 | **别人发「@全体成员」在事件里长什么样**（B3） | 群成员发一条 @全体 消息；用 debug 日志看 `GROUP_AT_MESSAGE_CREATE` / `GROUP_MESSAGE_CREATE` 的原始 `content`；再私信发一次 `/testat all` 作对照 | content 里的表示（如 `<@!all>` / `@everyone` / `@全体成员` / 其它）；机器人是否收到该事件；据此实现 B3 的精确拦截 |
