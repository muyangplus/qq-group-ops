...
| R17 | **机器人能不能自己 @全体**（决定活动「提醒全体」能不能做） | 群里发 `/testat all`（仅超管，会给全群发 11 条），观察**哪几条真的提醒了全群**（昵称/「全体成员」高亮、手机通知）；把汇总卡里对应编号回报 | 历史 5 种写法（`@everyone` / `<@!all>` / `<@!everyone>` / 纯文字 / 纯文本 `<@!all>`）全部不生效。2026-09-26 新增 R1 抓到的**官方入站原文形态** `<@all>`（卡片 + 纯文本）与纯文本 `@everyone` 共 8 条候选，重跑一次定位；若仍全部不生效 → 认定「机器人无法 @全体」，活动 `提醒@全体` 保持现状（明确提示由管理员手动 @） |
