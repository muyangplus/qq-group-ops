...
用 PowerShell 之类的脚本「按行区间剪切 + 改名 + 生成包装」可以快速搬领域，但有三条硬经验：

1. **参数名解析要兼容单行签名**：方法签名可能是单行（`public testCard(a: X, b: Y): T {`），
   只按「行首 `name:`」匹配会拿到 0 个参数，生成出 `fn(ctx, )` 这种坏包装；
   正确做法是先取签名括号内文本 `\(([^)]*)\)` 再拆参数。
2. **不要往目标模块追加整块 import**：目标模块若已有 import，重复绑定同名符号会直接编译失败
   （`Duplicate identifier`）。只补缺失的名字。
3. **一次只搬一个方法区间**，搬完立刻 `tsc --noEmit`；任何异常直接 `git checkout -- <文件>` 回滚，
