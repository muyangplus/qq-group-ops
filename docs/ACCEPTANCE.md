...
| J12 | 全量卡片 | 逐条执行 `/myperm`、`/bind`、`/whois`、`/profile`、`/perm list`、`/activity` | 每条都返回卡片（不是纯文本），带按钮与底部手动指令 |
...
| J15 | 回调动作有反馈 + 操作人 | 点 `/rules` 的开关/枚举、`/pending` 的「通过」、`/notify` 的订阅开关 | 卡片正文首行出现「结果：… · 操作人：<QQ号或短码>」，且状态真的变了 |
