...
| J34 | 报名落点（群内） | 资料完整的成员在群里点成员卡的「报名」并确认二次确认 | 群里回一张卡片：**首行 `<@!申请人>`**，正文只有 `报名成功 · 当前 X/Y`；**不出现**姓名/学号/班级/学院 |
| J35 | 报名落点（私聊） | 同一个人在私信里点活动卡片的「报名」 | 私聊回执可以包含姓名/学号/序号/人数（隐私字段只在私聊出现） |
| J36 | 报名失败只私信 | 直接点报名但资料不全 / 年级不符合 `allowYears` / 已报名 | 群里只回 `<@!申请人> 报名未通过，原因已私信`；**具体原因**（补全资料 / 仅限 … 级 / 已报名 / 已截止）只在私信里出现；关闭主动消息时群里只提示「请先私聊机器人再试」，仍不显示原因 |
| J37 | 候补与冻结名额 | 名额满后再报名；随后让已报名者「取消报名」 | 报名者收到「已进入候补 · 第 N 位」（群里只 @ 本人）；取消后管理卡出现「待释放名额 M」，点「释放名额」后候补第一位被**私信**通知「你已递补成功（当前 Y/Z）」，**不往群里发**；`waitlistPromotion auto` 时取消即自动递补 |
