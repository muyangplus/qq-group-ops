...
| H3 | 资料校验 | `/profile set id 12345`、`/profile set class 不存在的班级` | 分别返回「11 位」「不在班级库」的失败提示，不写入 |
...
| J20 | `/whois` 详情 | `/whois <#申请短码>` | 输出含群、申请人、理由、状态、申请/处理时间与处理人 |
| J21 | 智能 `/profile set` | 依次试 `/profile set 材化2211 张三 22123456789`、`/profile set 张三-22123456789-材化2211`、`/profile set 材化2211张三22123456789`、`/profile set 班级=材化2211 姓名=张三 学号=22123456789` | 四种写法都写入成功并回显「识别到：…」；学院/年级由班级自动带出 |
...
| J23 | `/whois profile` | 超管发 `/whois profile <QQ号>`、`/whois profile #用户短码`、`/whois profile <未填资料的QQ号>` | 输出 userId / QQ号 / 短码 + 姓名 / 学号 / 班级 / 学院；未填资料显示「尚未填写」；群管理员执行提示权限不足 |
