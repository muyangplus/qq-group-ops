...
- 导航 / 查看 / 翻页按钮是**回调按钮**（点击即出下一张卡）；执行动作是**指令按钮**
  （点击=发送指令，权限与手输一致）；
- 列表分页：每页固定条数 + 回调翻页，正文同时给出 `+页码` 指令，例如
  `/pending +2`、`/pending 654321 +2`、`/audit 20 +2`；
- 开关与枚举（`/rules` 的规则开关与入群决策、`/notify` 的订阅开关）都是回调：
  点击即自动生效并回一张刷新后的卡片，正文会标明「操作人」；
- 需要自由文本/数值的动作（`/reject <ID> <原因>`、`/rules set keywords ...`、`/bind`）
  仍是指令按钮或手输指令；
- 按钮未开通时自动降级为纯文本，正文里的指令可直接复制执行。
