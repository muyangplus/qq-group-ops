...
/whois profile <QQ号|userId|#短码>       # 超管查询个人资料（姓名/学号/班级/学院/年级）
```
...
- **唯一例外是 `/whois`**（超管）：`/whois #M7K2Q9` 会显示短码对应的类型与真实系统 id，`/whois <QQ号>` 显示对应 `userId` 与短码；**不带参数时直接查当前上下文**（群聊=当前群、私聊=你自己）；`/whois profile <QQ号|userId|#短码>` 额外给出该用户的个人资料（姓名/学号/班级/学院/年级），同样仅限全局超管（资料属个人信息，不开放给群管理员）；
