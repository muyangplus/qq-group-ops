...
- **`/testat`：@ 渲染真机自检（仅全局超管，群里执行）**：官方内嵌格式（`<@!openid>` 与
  `@everyone`）**只在 `content` 里生效**，而 Markdown 卡片（`msg_type=2`）的 payload 不带
  `content`，所以「卡片里的 @ 到底有没有效果」必须实测。`/testat` 依次发 3 条：纯文本 @、
  Markdown 首行 @、Markdown 正文中间 @；`/testat all` 额外发一条纯文本 `@everyone`
  （会真的 @ 全群，按钮带二次确认）。发完回一张汇总卡，记录每条是否发送成功与
  `@everyone` 失败原因，按回复结果统一后续所有 @ 的实现方式。为此新增
  `RichMessageSender.sendPlainToGroup()/sendPlainToUser()`（纯文本通道，不带 markdown 选项）。
### 已知限制
