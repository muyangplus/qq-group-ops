...
export const HELP_TOPICS: readonly HelpTopic[] = [
  ...
  },
  {
    ...
    body: () => [
      "用法：",
      "  /notify                打开统一通知订阅菜单",
      "  /notify test [频道]    给自己发一张该频道的测试卡片（频道：join / punish / activity）",
      "",
      "统一菜单（一张卡管三个频道）：",
      "  · 入群申请    有新的待处理申请（或自动处理结果）时私信你",
      "  · 处罚与申诉  关键词处罚、申诉派发与申诉结果私信你",
      "  · 活动通知    群里有新活动发布时私信你",
      "  · 每行三个按钮：「<频道> 本群」/「<频道> 全部」/「测试」；点一下即切换并刷新同一张卡",
      "  · 「全部」= 所有装了机器人的群（绑定是全局的：在一个群绑过 QQ 号即可）",
      "",
      "权限（与推送对象一致，避免「订阅成功但永远收不到」）：",
      "  · 入群申请：该群群管理员及以上",
      "  · 处罚与申诉：该群审核员及以上",
      "  · 活动通知：不限权限；「全部」需要已绑定 QQ 号",
      "",
      "推送内容：",
      "  · 申请卡片是 Markdown + 底部「同意 / 拒绝」按钮，点击等于发送 /approve、/reject 并二次确认",
      "  · 处罚卡片上可直接解除处罚、改禁言时长、踢出、拉黑本群或全局（全局仅超管）",
      "  · 卡片正文只显示绑定的群号/QQ号；未绑定时才显示内部 openid（/whois 例外）",
      "  · 只推送仍需人工处理的申请；自动通过/拒绝的不会打扰",
      "",
      "说明：",
      "  · 三个频道共用一张订阅表，重启不丢；同一申请对同一个人只推送一次",
  ...
  },
  {
    ...
    body: () => [
      ...
      "",
      "说明：",
      "  · 处罚记录保存规则说明与动作；消息原文**默认不保存**，可用 `/rules set rawMessageRetentionDays <天数>`",
      "    开启短期保留（单行截断 ≤200 字）——原文只出现在私信卡片里，群内「处罚通知」卡不含规则名与原文；",
      "  · 申诉**只对本人开放**：同一处罚**待处理期间不能重复提交**，审核员给出通过 / 驳回后才能再提交；",
      "  · 申诉派发：**所有管理员**都会收到，**审核员之间轮单**（一次一位，超时转下一位）；",
      "    处理结果会私信通知申诉人本人，其他审核员收到「申诉已处理」同步卡；",
