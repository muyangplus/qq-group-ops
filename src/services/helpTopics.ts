...
export const HELP_TOPICS: readonly HelpTopic[] = [
  ...
  },
  {
    ...
    body: () => [
      ...
      "",
      "说明：",
      "  · 官方内嵌格式（`<@!openid>` 与 `@everyone`）**只在 `content` 里生效**，",
      "    而 Markdown 卡片（`msg_type=2`）不带 `content`，所以卡片里的 @ 是否有效必须真机实测；",
      "  · 发完会给一张汇总卡，请回复「哪几条真的 @ 到了你」，据此统一后续所有 @ 的实现方式；",
      "  · `@everyone` 需要机器人拥有发送 @所有人 消息的权限，失败会在汇总卡里给出原因。",
    ],
  },
  {
    name: "bind",
